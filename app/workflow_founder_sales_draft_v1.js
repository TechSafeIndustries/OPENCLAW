/**
 * OpenClaw — Founder Sales Draft Workflow v1
 * -------------------------------------------
 * Deterministic workflow runner for the Founder Mode sales-draft sequence.
 * Chains three existing CLIs in order with --no-stub enforced:
 *
 *   1. openclaw:run  — requests/sales_draft.json --founder --new-session
 *   2. artifacts:latest NEW_SESSION_ID --no-stub
 *   3. tasks:next    NEW_SESSION_ID --no-stub --owner cos
 *
 * Does NOT write to DB directly — all audit writes happen inside the called CLIs.
 *
 * Usage:
 *   node app/workflow_founder_sales_draft_v1.js [--dry-run] [--owner <agent>]
 *
 * Flags:
...
const REQUEST_FILE = path.join(ROOT, 'requests', 'sales_draft.json');

// ── Derive child env ──────────────────────────────────────────────────────────
// kimi-stub flag forces stub mode so the workflow can be smoke-tested without
// a live Moonshot API key.  The shell environment always wins over .env.
...
function runCLI(label, nodeArgs, timeoutMs) {
    ...
    if (result.error) {
        // spawnSync-level error (timeout, ENOENT, etc.)
        return {
            ok: false,
            label,
    ...
    // Strip npm wrapper lines ("> openclaw-gateway@... tasks:...") if present
    // Real CLIs write JSON directly to stdout/stderr, but when invoked via npm
    // the npm header lands on stderr; via node directly it's clean.
    let parsed = null;
    try {
